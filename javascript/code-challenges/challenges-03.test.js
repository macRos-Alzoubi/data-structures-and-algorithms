...
const getStatName = (arr, minBaseStat) => getBaseStatGreaterThan(arr, minBaseStat).map(obj => obj.stat.name);
...
const getCharactersWithoutChildren = arr => arr.filter(obj => !obj.children);
...
const evenOddNumericValues = arr => typeNum(arr).map(item => (item%2)? 'odd' : 'even');
