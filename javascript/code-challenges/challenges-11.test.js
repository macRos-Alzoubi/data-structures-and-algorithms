...
const count = (target, input) => input.reduce((acc, item) => {
  let filtered = item.filter( subItem => subItem === target);
  acc += filtered.length;
  return acc;
}, 0);
...
const totalSum = (input) => input.reduce((acc, item) => {
  let sum =item.reduce((acc, subItem) => { acc += subItem; return acc;}, 0);
  acc += sum;
  return acc;
}, 0);
...
const divisibleByFiveTwoToThePower = (input) => input.map(item => {
  let resultArray = item.reduce((acc, subItem) => {
    typeof subItem === 'number' && !(subItem%5) ? acc.push(subItem) : null;
    return acc;
  } , []);

  return resultArray.map(item => Math.pow(2, item));
});
...
let findMaleAndFemale = (data) => data.reduce((acc, item) => {
  item.gender === 'female' || item.gender === 'male' ? acc.push(item.name) : null;
  return acc;
}, []).join(' and ');
...
let findShortest = (data) => data.reduce((acc, item) => {
  acc = Number(item.height) <= Number(acc.height) ? item : acc;
  return acc;
}, data[0]).name;
