...
const standardizePhoneNumbers = (arr) => arr.map(item =>{
  let newStr = '';
...
const allHappy = (arr) => arr.length === arr.filter(item => item.includes(':)')).length;
...
const findEvery = (arr, target) => arr.length === arr.filter(item => item.includes(target)).length;
...
const unenrollBrook = (arr) => arr.map(item => item.filter(subItem => !subItem.includes('Brook')));
...
const sortByDay = (arr) => {
  ...
  daysOfWeek.forEach((item) =>{
    newArray.push(arr.filter(subItem => subItem.includes(item)));
  });
  return newArray;
};
/* ------------------------------------------------------------------------------------------------
...
const characterByIndex = (arr) => arr.map((item, idx) => item[idx]);
