...
const screenForNames = (arr) => arr.filter(item => /^([MD]r|[MD]rs|Ms).\s?[a-zA-Z]\w*/g.test(item));
...
const toTitleCase = (arr) => arr.map(item => item[0].toLocaleUpperCase() + item.slice(1, item.length) );
/* ------------------------------------------------------------------------------------------------
CHALLENGE 3
...
let biggerThanLuke = (arr) => arr.reduce((acc, item) => {
  if(item.mass > 77)
    acc.push(item.name);
  return acc;
}, []).join(' - ');
