...
------------------------------------------------------------------------------------------------ */
const toLastNames = people => people.map( peopleObj => `${peopleObj.firstName} ${peopleObj.lastName}`);
...
const addValues = (arr) => arr.reduce((acc, item)=> acc += item, 0);
...
const addPurchases = (arr) => arr.reduce((acc, itemObj) => acc += itemObj.purchasePrice, 0);
...
const returnNames = arr => arr.reduce((acc, item) =>{ acc.push(item.name); return acc;}, []);
...
// [char].concat(acc) From => https://stackoverflow.com/questions/40363417/javascript-using-reduce-to-reverse-an-array
const reversedString = str => str.split('').reduce((acc, char) => [char].concat(acc), []).join('');
//
...
const countNumberOfChildren = arr => arr.reduce((acc, obj) => obj.children ? acc += obj.children.length : acc, 0);
...
const calculateAverage = arr => { const obj = arr.reduce((acc, num) => {
  acc.count += 1;
  acc.sum += num;
  return acc;
}, { count: 0, sum: 0});
...
const countPrimeNumbers = arr => arr.reduce((acc, num) => isPrime(num)? ++acc : acc, 0);
...
const extractStat = (statName, arr) => arr.reduce((acc, obj) => obj.stat.name === statName ? obj : (acc.stat.name === statName ? acc : null), {});
...
const extractChildren = arr => arr.filter(obj => obj.name.includes('a')).reduce((acc, obj) => obj.children ? acc.concat(obj.children) : acc , []);
