...
E.g. [4,2,7,5,9,2] -> 9
------------------------------------------------------------------------------------------------ */
const maxInArray = (arr) => arr.reduce((acc, item) => {
  acc = acc < item ? item : acc;
  return acc;
}, arr[0]);
...
const checkValues = (obj, value) => Object.values(obj).filter(item => value === item).length ? true : false;
...
const updateNumbers = (obj) => Object.keys(obj).reduce((acc, item) => {
  acc.push(`${item}: ${obj[item]}`);
  return acc;
}, []);
...
//const hasChildrenValues = (arr, character) => arr.filter(item => item.name === character)[0].children;
const hasChildrenValues = (arr, character) => Object.keys(arr.filter(item => item.name === character)[0]).length > 3;
...
const hasChildrenEntries = (arr, character) => {
  const res = arr.filter(item => item.name === character);
  return res.length ? (res[0].children ? true : false) : false;
};
...
const totalCharacters = arr => arr.reduce((acc, item) => {
  acc = item.spouse ? acc + 2 : acc + 1;
  acc = item.children ? acc += item.children.length : acc;
  return acc;
}, 0);
...
const houseSize = (arr) => {
  ...
  arr.reduce((acc, item) => {
    const obj = {};
    obj.house = item.house;
    obj.members = 0;
    obj.members += item.spouse ? obj.members + 2 : obj.members + 1;
    obj.members += item.children ? item.children.length : obj.members;
    acc.push(obj);
    return acc;
  }, sizes);
  return sizes;
...
const houseSurvivors = arr => {
  ...
  arr.reduce((acc, item) => {
    ...
    obj.house = item.house;
    obj.members = 0;
    obj.members += (deceasedSpouses.includes(item.name) || deceasedSpouses.includes(item.spouse)) ? obj.members : obj.members + 1;
    obj.members += item.spouse ? obj.members + 1 : obj.members;
    obj.members += item.children ? item.children.length : obj.members;
    acc.push(obj);
    return acc;
  }, survivors);
