...
const removeThree = (idx, arr) => arr.slice(0, idx).concat(arr.slice(idx + 3));
...
const listFoods = (recipe) => {
  let result = [];
  recipe.ingredients.forEach(ingItem => result.push(ingItem.split(' ').slice(2).join(' ')));
  return result;
};
...
const removeEvenValues = (arr) => arr.forEach((item, idx) => !(item%2) ? arr.splice(idx, 1) : null);
...
const removeLastCharacters = (str, numberOfCharacters) => numberOfCharacters > 0 ? str.slice(0, numberOfCharacters * -1) : str;
...
const totalSumCSV = (str) => str.split(',').reduce((acc, item)=> {acc += Number(item); return acc;},0);
...
const extractVowels = (str) => {
  let withoutVowels = removeVowels(str);
  let vowelsList = [];
  for(let i = 0; i < str.length; i++){
  ...
  return [withoutVowels, vowelsList.sort((a, b) => a < b ? -1 : (a > b ? 1 : 0)).join('')];
};
